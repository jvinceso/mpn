...
class PHPJasperXML {
    ...
    /**
     * Retorna Valores True de la conexion
     */
        if($cndriver=="mysql") {
...
//                            echo "Barcode: $code,position: $textposition <br/><br/>";
    ...
    public function element_break($data) {
                $this->pointer[]=array("type"=>"break","hidden_type"=>"break");
...
//        $b=$data->subreportParameter;
    ...
    print "<pre>".print_r($this->arrayfield,TRUE)."</pre>";
    print "<pre>".print_r($this->arraysqltable,TRUE)."</pre>";
    }

    public function dataToArray($result){
        // print "<pre>".print_r($result,TRUE)."</pre>";
        $this->m=0;
        $this->arraysqltable = $result;
        // foreach ($result as $row) {
        //     foreach($this->arrayfield as $out) {
        //         // print "<pre>".print_r($row["$out"],TRUE)."</pre>";
        //         // print "<pre>".print_r($this->m,TRUE)."</pre>";
        //         $this->arraysqltable[$this->m]["$out"]=$row["$out"];
        //         // print "<pre>".print_r($this->arraysqltable,TRUE)."</pre>";
        //     }
        //     $this->m++;
        // }
        // print "<pre>".print_r($this->arraysqltable,TRUE)."</pre>";exit();
    }
    ...
    public function outpage($out_method="I",$filename="") {
            ...
            if($this->pdflib=="TCPDF") {
                if($this->arrayPageSetting["orientation"]=="P")
                    $this->pdf=new TCPDF(
                        $this->arrayPageSetting["orientation"],
                        'pt',
                        array(
                            intval($this->arrayPageSetting["pageWidth"]),
                            intval($this->arrayPageSetting["pageHeight"])
                        ),
                        true
                    );
                else
...
//                $arrdata[$num]=str_replace('"',"",$out);
    ...
    public function includeSubReport($d,$arrdata,$current_y){ 
               ...
               $srxml=  @simplexml_load_file(FCPATH."reportes_design/".$d['subreportExpression']);
               // print_p($d['subreportExpression']);
               // echo "wdfsdf";
               // print_p($srxml);
               // exit();
